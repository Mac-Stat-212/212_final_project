...
```{r data}
dat_wider <- read_csv('../data/long_data_sub.csv')
```
...
```{r}
volume_cat_big <- full_data_volume %>% 
  group_by(stock_name) %>% 
...
```{r}
volume_cat_small <- full_data_volume %>% 
  group_by(stock_name) %>% 
  summarize(med_volume = median(Volume)) %>% 
  filter(med_volume <= 1000000)
```

```{r}
small_stock_data <- full_data %>% 
  filter(stock_name %in% volume_cat_big$stock_name) %>%
  filter(!is.na(Low))
```
...
```{r, include=FALSE}
dat_rounded_low <- mutate(dat_wider_low, rounded=cut(Low - floor(Low), seq(0, 1, by=0.1), right=FALSE))
...
```{r}
price_cats <- c("0-0.9", "0.1-0.19", "0.2-0.29", "0.3-0.39", "0.4-0.49", "0.5-0.59", "0.6-0.69", "0.7-0.79", "0.8-0.89", "0.9-0.99")
low_ranges %>% 
  ggplot() +
  geom_col(aes(x = rounded, y = n)) +
  labs(title = "Anchoring Bias Visible in Cent Prices", x = "Cent Price", y = "Recorded Lows") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 30)) +
  scale_x_discrete(label = price_cats)
```

```{r}
low_data <- big_stock_data %>%
  ...
  mutate(low_cat = as.numeric(low_cat)/100)
```

```{r}
big_stock_data %>%
  # filter(stock_name == "MSFT") %>%
  # filter(Size == "big") %>% 
  mutate(Low = round(Low, 2)) %>%
  # filter(Low < 500) %>% 
  mutate(Low = as.character(Low)) %>% 
  ...
  labs(title = "", x = "Cent Price", y = "Recorded Lows") +
  theme_minimal()
```
...
big_stock_data %>%
  ...
  mutate(high_cat = ifelse(str_detect(High, "\\..$"), paste0(str_sub(High, -1, -1), "0"), str_sub(High, -2, -1))) %>%
  mutate(high_cat = ifelse(str_detect(High, "\\."), high_cat, 0)) %>%
  group_by(high_cat) %>% 
  count() %>% 
  filter(!is.na(high_cat)) %>% 
  mutate(high_cat = as.numeric(high_cat)/100) %>% 
  # filter(low_cat != 0) %>% 
  # mutate(low_cat = as.character(low_cat)) %>% 
  ggplot() +
  geom_line(aes(x = high_cat, y = n)) +
  labs(title = "", x = "Cent Price", y = "Recorded Highs") +
  theme_minimal()
```
