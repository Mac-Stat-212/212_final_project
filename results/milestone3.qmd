...
```{r}
small_stock_data <- big_stock_data %>% 
...
```{r}
stock_names <- big_stock_data %>% filter(Size == "big") %>% pull(stock_name) %>% unique()
  
selectInput("stock_names", "Stock:", choices = stock_names)
selectInput("metrics", "Metric:", choices = metrics)
